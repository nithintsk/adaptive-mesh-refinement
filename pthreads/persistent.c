...
int main (int argc, char *argv[])
{
    ...
    MAIN_MAX_DSV = (double*)malloc(NUM_THREADS*sizeof(double));
    MAIN_MIN_DSV = (double*)malloc(NUM_THREADS*sizeof(double));
    ...
    tempmin = (double*) malloc(NUM_THREADS * sizeof(double));
    tempmax = (double*) malloc(NUM_THREADS * sizeof(double));
	...
	for (tn=0; tn<NUM_THREADS; tn++) {
		...
		targs[tn].tid = tn;
		targs[tn].max_DSV = tempmin;
		targs[tn].min_DSV = tempmax;
...
void* compute_commit_dsv(void* arg1) {
    ...
    do {   
        ...
        // Once all threads commit
        pthread_barrier_wait(&mybarrier); 
        
        pthread_barrier_wait(&mybarrier);
        pthread_mutex_lock(&minlock);
        if(LOCAL_MIN_DSV < MIN_DSV) MIN_DSV = LOCAL_MIN_DSV;
        if(LOCAL_MAX_DSV > MAX_DSV) MAX_DSV = LOCAL_MAX_DSV;
