...
int main (int argc, char *argv[])
{
	...
	struct Box *hboxes;
	hboxes = (struct Box*)malloc(NUM_BOXES * (sizeof(struct Box)));
    ...
    clock_time = clock();
    int count;
    printf("Reading Values from compute and commit");
    count = cuda_compute_commit_dsv(hboxes,NUM_BOXES,AFFECT_RATE,EPSILON,MIN_DSV,MAX_DSV);
    printf("\nFinished Reading Values from compute and commit\n");
